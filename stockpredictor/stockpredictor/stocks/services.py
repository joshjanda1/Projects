...
def get_stock(symbol='SPX'):
	...
	intermediate_data = pd.concat([data, sma_, macd_, rsi_, adx_], axis=1, sort=True)
		
	data = data.reset_index()#reset index so standard..
	data['Open'], data['High'], data['Low'], data['Volume'], data['Close'] = data['1. open'], data['2. high'], data['3. low'], data['5. volume'], data['4. close']
	data = data.drop(['1. open', '2. high', '3. low', '5. volume'], axis=1)#gets conventional names for labels
